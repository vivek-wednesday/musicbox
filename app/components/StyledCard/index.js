...
import { BackwardOutlined, PlayCircleOutlined, ForwardOutlined, CustomerServiceOutlined, PauseCircleOutlined } from '@ant-design/icons';
...
function StyledCard(props) {
  ...
  function controlAudio(val, type) {
    ...
    if (type === "backward") {
      ...
      ele.play(); setIsPlaying(true);
      setCurrentSong(val);
      showProgress(ele);
    } else if (type === "pause") {
      ele.pause(); setIsPlaying(false);
      clearInterval(itrvl);
    }
  }

  function showProgressBar(id) {
    if (currentSong === id) { return <Progress percent={progress} showInfo={false} /> }
  }

  return (
    (musicData === undefined) ? <div>Nothing here</div> : isEmpty(musicData.results) ? <div>Nothing here</div> :
      musicData.results.slice(0, 10).map((item, index) => {
        ...
        return <Col key={index}>
          <Card
            ...
            actions={[
              <BackwardOutlined key="backward" style={{ fontSize: 35, color: colors.secondaryBtn }} onClick={() => {
                controlAudio(index, "backward")
              }} />,
              <>
                {
                  isPlaying ? (currentSong === index) ?
                    <PauseCircleOutlined key="play" style={{ fontSize: 40, color: colors.primaryBtn }} onClick={() => {
                      controlAudio(index, "pause")

                    }} /> : <PlayCircleOutlined key="play" style={{ fontSize: 40, color: colors.secondaryBtn }} /> :
                    <PlayCircleOutlined key="play" style={{ fontSize: 40, color: colors.primaryBtn }} onClick={() => {
                      controlAudio(index, "play")
                    }} />
                }</>,
              <ForwardOutlined key="forward" style={{ fontSize: 35, color: colors.secondaryBtn }} onClick={() => {
