...
export function MusicDetails({musicData, pathName, intl}) {
  ...
  return (
    <StyleDiv>
    {
      musicData.results.map((item, index) => item.trackId === +Path ? 
        <Card
        ...
        title={ <Title>{item.trackName}</Title> }>
          ...
          <Para data-testid="rating">Rating: {item.contentAdvisoryRating ?? intl.formatMessage({id: "unrated", defaultMessage: "Unrated"})}</Para>
        </Card>
      : null )
    }
    </StyleDiv>
